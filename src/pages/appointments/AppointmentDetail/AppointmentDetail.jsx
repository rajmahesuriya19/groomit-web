...
import { getAppointmentDetail, getRebookDetail } from '@/utils/store/slices/appointments/appointmentsSlice';
...
const AppointmentDetail = () => {
    ...
    const isRebookPage = location.pathname.includes('view-rebook-confirmation-session');
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                if (isRebookPage) {
                    await dispatch(getRebookDetail(id));
                } else {
                    await dispatch(getAppointmentDetail(id));
                }
            } finally {
                hideLoader();
            }
        ...
        <>
            ...
            <div className="px-5 py-6 grid grid-cols-1 md:grid-cols-[minmax(0,1.25fr)_auto_minmax(0,1fr)] gap-8">
                ...
                <div className="space-y-4">
                    {isRebookPage ? (
                        <RebookConfirmationDetail selectedAppointment={selectedAppointment} />
                    ) : (
                        <>
